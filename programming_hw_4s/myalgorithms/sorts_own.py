...
def merge_sort_1(arr: list):
    ...
    for i in range(len(sorted_list)): arr[i] = sorted_list[i]


def merge_sort_2(arr: list):
    def merge(left_arr: list, right_arr: list):
        index_l = index_c = index_r = 0
        length_l, length_r = len(left_arr), len(right_arr)

        sorted_arr = [0] * (length_l + length_r)

        while index_l < length_l and index_r < length_r:
            if left_arr[index_l] <= right_arr[index_r]:
                sorted_arr[index_c] = left_arr[index_l]
                index_l += 1
                index_c += 1
            else:
                sorted_arr[index_c] = right_arr[index_r]
                index_r += 1
                index_c += 1
